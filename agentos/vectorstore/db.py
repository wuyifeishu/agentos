...
class BaseVectorStore:
    """向量存储基类。"""
    def add(self, texts: list[str], metadatas: list[dict] | None = None, ids: list[str] | None = None) -> list[str]: ...
...
class FAISSVectorStore(BaseVectorStore):
    ...
    def _fallback_search(self, q_vec, top_k):
        ...
        return [VectorEntry(id=rid, text=text, metadata=meta, score=float(s))
    ...
    def search(self, query: str, top_k: int = 5) -> list[VectorEntry]:
        ...
        q_vec = self._embed([query])[0]
        import numpy as np
        D, I = self._index.search(np.array([q_vec], dtype=np.float32), min(top_k, self.count()))
        results = []
    ...
    def add(self, texts: list[str], metadatas: list[dict] | None = None, ids: list[str] | None = None) -> list[str]:
        if not self._collection:
            ...
            for i, t in enumerate(texts):
                self._fallback_store[ids[i]] = {"text": t, "metadata": metadatas[i] if metadatas else {}}
            return ids
