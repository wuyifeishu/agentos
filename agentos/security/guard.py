    ...
    def check_toxicity(self, content: str) -> list[GuardResult]:
        ...
        for category, patterns in self._toxicity_re.items():
            for i, pattern in enumerate(patterns):
                if pattern.search(content):
                    severity = Severity.CRITICAL if category in ("self_harm", "illegal") else Severity.HIGH
                    results.append(GuardResult(
                        passed=False,
                        action=GuardAction.BLOCK,
                        severity=severity,
...
class InputGuard:
    ...
    def guard(self, user_input: str, redact_pii: bool = True) -> GuardChainResult:
        ...
        if self.deny_empty and (not user_input or not user_input.strip()):
            results.append(GuardResult(
                passed=False, action=GuardAction.BLOCK,
                severity=Severity.LOW, rule_name="empty_input",
        ...
        if self.max_input_length > 0 and len(user_input) > self.max_input_length:
            results.append(GuardResult(
                passed=False, action=GuardAction.BLOCK,
                severity=Severity.LOW, rule_name="input_too_long",
        ...
        if redact_pii:
            ...
            if items:
                ...
                results.append(GuardResult(
                    passed=True, action=GuardAction.REDACT,
                    severity=Severity.MEDIUM, rule_name="pii_redacted",
...
class OutputGuard:
    ...
    def __init__(
        ...
        self.pii = pii_detector or PIIDetector(auto_redact=True)
        self.safety = safety_filter or ContentSafetyFilter(block_injection=False)  # No injection check on output
    ...
    def guard(self, agent_output: str) -> GuardChainResult:
        ...
        if self.deny_empty and (not agent_output or not agent_output.strip()):
            results.append(GuardResult(
                passed=False, action=GuardAction.BLOCK,
                severity=Severity.MEDIUM, rule_name="empty_output",
        ...
        if items:
            ...
            results.append(GuardResult(
                passed=True, action=GuardAction.REDACT,
                severity=Severity.HIGH, rule_name="pii_leak_prevented",
        ...
        if self.block_system_prompt_leak:
            ...
            for i, pattern in enumerate(leak_indicators):
                if re.search(pattern, current_content):
                    results.append(GuardResult(
                        passed=False, action=GuardAction.BLOCK,
                        severity=Severity.CRITICAL, rule_name=f"prompt_leak_{i}",
...
class GuardPipeline:
    ...
    def _log(self, stage: str, result: GuardChainResult) -> None:
        guard_results = [
            {"rule": r.rule_name, "passed": r.passed, "action": r.action.value,
...
def create_strict_guard() -> GuardPipeline:
    ...
    return GuardPipeline(
        ...
        output_guard=OutputGuard(pii_detector=pii, safety_filter=safety, block_system_prompt_leak=True),
    )
...
def create_permissive_guard() -> GuardPipeline:
    ...
    return GuardPipeline(
        ...
        output_guard=OutputGuard(pii_detector=pii, safety_filter=safety, block_system_prompt_leak=False),
    )
