...
class ProductionAgent:
    ...
    def __init__(
        ...
        *,
        ...
        # cache — wrap provider transparently
        if cache and self._config.enable_cache:
            self._cache = cache
            self._provider = cache.wrap(provider)
        else:
            self._cache = None
    ...
    def run(self, task: str) -> AgentResult:
        ...
        if self._cache and result.success:
            # track estimated cost savings from cache
            self._cache._stats.total_cost_saved_usd += route_result.estimated_cost
    ...
    def cache_stats(self) -> Any | None:
        ...
        hits, misses, fuzzy_hits, exact_hits, evictions,
        total_cost_saved_usd, total_entries, hit_rate, fuzzy_hit_rate.
        """
    ...
    def cache_savings(self) -> float:
        ...
        if not self._cache:
            return 0.0
        return self._cache.stats.total_cost_saved_usd
