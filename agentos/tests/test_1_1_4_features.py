...
class TestMiddlewarePipeline:
    ...
    async def test_empty_pipeline_allows(self):
        from agentos.core.middleware import MiddlewareContext, MiddlewarePhase, MiddlewarePipeline
...
class TestRunCostSession:
    ...
    def test_total_tokens(self):
        ...
        session.records.append(
            UsageRecord(model="m", input_tokens=100, output_tokens=50, cost_usd=0.0)
        )
        session.records.append(
            UsageRecord(model="m", input_tokens=200, output_tokens=100, cost_usd=0.0)
        )
...
class TestCostTrackerEnhanced:
    ...
    def test_record_with_session(self):
        ...
        rid = tracker.start_session()
        tracker.record(
            "deepseek-v3.1", {"prompt_tokens": 1000, "completion_tokens": 500}, run_id=rid
        )
        tracker.record(
            "deepseek-v3.1", {"prompt_tokens": 500, "completion_tokens": 200}, run_id=rid
        )
        session = tracker.end_session(rid)
        assert session.call_count == 2
        assert session.total_cost > 0
    ...
    def test_cost_by_session(self):
        ...
        tracker.record(
            "deepseek-v3.1", {"prompt_tokens": 1000, "completion_tokens": 100}, run_id=r1
        )
