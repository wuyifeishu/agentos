...
class FilesystemServer:
    ...
    def __init__(self, allowed_paths: list[str] | None = None):
        self._allowed_paths = [
            Path(p).resolve() for p in (allowed_paths or [os.getcwd(), str(Path.home())])
        ]
    ...
    def _handle_search_files(self, path: str, pattern: str) -> list[dict]:
        ...
        return sorted(
            [
                {"name": i.name, "path": str(i), "size": i.stat().st_size, "is_dir": i.is_dir()}
...
class WebFetchServer:
    ...
    def get_tools(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "name": "check_url",
                "description": "HEAD request to verify URL accessibility",
                "inputSchema": {
                    "type": "object",
    ...
    def _validate_url(url: str) -> None:
        ...
        if p.scheme not in ("http", "https"):
            raise ValueError(f"Invalid scheme: {p.scheme}")
    ...
    def _handle_fetch_json(self, url: str, headers: dict | None = None) -> Any:
        self._validate_url(url)
        ...
        try:
            with urllib.request.urlopen(
                urllib.request.Request(url, headers=h), timeout=self._timeout
            ) as r:
        ...
        try:
            with urllib.request.urlopen(req, timeout=self._timeout) as r:
                return {
                    "url": url,
                    "status": r.status,
                    "accessible": 200 <= r.status < 400,
                    "content_type": r.headers.get("Content-Type", ""),
                    "content_length": r.headers.get("Content-Length", ""),
                }
        except urllib.error.HTTPError as e:
            return {"url": url, "status": e.code, "accessible": False, "reason": e.reason}
        except Exception as e:
...
class MemoryServer:
    ...
    def __init__(self, storage_path: str = ""):
        ...
        if self._path.exists():
            try:
                for e in json.loads(self._path.read_text()).get("entries", []):
    ...
    def get_tools(self) -> list[dict[str, Any]]:
        return [
            {
                "name": "store_memory",
                "description": "Store a fact/memory",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "content": {"type": "string"},
            ...
            {
                "name": "search_memory",
                "description": "Search memories by keyword/category/tags",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "query": {"type": "string"},
            ...
            {
                "name": "update_memory",
                "description": "Update memory content/metadata",
                "inputSchema": {
                    "type": "object",
                    "properties": {
    ...
    def _handle_store_memory(
        self, content: str, category: str = "general", tags: list = None, metadata: dict = None
    ) -> dict:
        ...
        if entry:
            ...
            if tags:
                entry.setdefault("tags", []).extend(
                    t for t in tags if t not in entry.get("tags", [])
                )
            if category != "general":
    ...
    def _handle_retrieve_memory(self, memory_id: str) -> dict | None:
        return self._entries.get(memory_id)

    def _handle_search_memory(
        self, query: str = "", category: str = None, tags: list = None, limit: int = 20
    ) -> list[dict]:
        results = []
        for e in self._entries.values():
            if category and e.get("category") != category:
                continue
            if tags and not all(t in e.get("tags", []) for t in tags):
                continue
            if query and query.lower() not in e.get("content", "").lower():
                continue
            results.append(
                {
                    "id": e["id"],
    ...
    def _handle_list_categories(self) -> dict[str, int]:
        c: dict[str, int] = {}
        for e in self._entries.values():
            c[e.get("category", "general")] = c.get(e.get("category", "general"), 0) + 1
    ...
    def _handle_update_memory(
        self, memory_id: str, content: str = None, category: str = None, tags: list = None
    ) -> dict:
...
class SearchServer:
    ...
    def _handle_web_search(self, query: str, max_results: int = 10) -> list[dict]:
        ...
        for m in re.finditer(
            ...
            results.append(
                {"title": title, "url": link, "snippet": snippet, "source": "duckduckgo"}
            )
        return results
...
class GitServer:
    ...
    def __init__(self, repo_path: str = ""):
        ...
        self._git: str | None = shutil.which("git")
    ...
    def get_tools(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {"remote": {"type": "boolean", "default": False}},
                },
            },
        ]
...
class CodeServer:
    ...
    def get_tools(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "name": "run_shell",
                "description": "Execute a one-liner bash command",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "command": {"type": "string"},
            ...
            {
                "name": "lint_code",
                "description": "Basic code linting (syntax check)",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "code": {"type": "string"},
    ...
    def _handle_lint_code(self, code: str, language: str = "python") -> dict:
        if language == "python":
            try:
                ...
                return {
                    "valid": False,
                    "errors": [{"line": e.lineno, "offset": e.offset, "message": e.msg}],
                }
...
class TextServer:
    ...
    def get_tools(self) -> list[dict[str, Any]]:
        return [
            {
                "name": "count_tokens",
                "description": "Estimate token count (OpenAI tiktoken-style approximate)",
                "inputSchema": {
                    "type": "object",
            ...
            {
                "name": "summarize_text",
                "description": "Simple extractive text summarization",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "text": {"type": "string"},
            ...
            {
                "name": "format_json",
                "description": "Format/validate/prettify JSON",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "text": {"type": "string"},
...
class BuiltinMCPRegistry:
    ...
    def tool_count(self) -> int:
        return sum(len(s.get_tools()) for s in self._servers.values())
...
def create_default_registry(
    ...
    if allowed_paths is None:
        allowed_paths = [os.getcwd(), str(Path.home())]
