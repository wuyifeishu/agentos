...
def quick_eval(agent_fn: Callable, dataset: GoldenDataset, config: Optional[EvalConfig] = None) -> List[ScoreDetail]:
...
def bleu_score(reference: str, candidate: str, n: int = 4, smoothing: bool = False) -> float:
    ...
    for k in range(1, n + 1):
        ...
        cand_ngrams = Counter(tuple(cand_tokens[i:i + k]) for i in range(len(cand_tokens) - k + 1))
