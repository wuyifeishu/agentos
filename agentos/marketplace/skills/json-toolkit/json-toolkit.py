...
def run(action: str, file_path: str = "", json_str: str = "", query: str = "",
    ...
    try:
        if action == "validate":
            data = _load()
            if data is None: return "[json-toolkit] 无有效输入"
        ...
        if action == "format":
            ...
            if output_path:
                with open(output_path,"w",encoding="utf-8") as f: f.write(formatted)
                return f"已格式化写入: {output_path}"
            return formatted
        if action == "query":
            data = _load()
            if data is None: return "[json-toolkit] 无有效输入"
            if not query: return "[json-toolkit] query 不能为空"
