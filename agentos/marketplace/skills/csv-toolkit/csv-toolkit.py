...
def run(action: str, file_path: str = "", query: str = "", output_path: str = "",
        ...
        if action == "stats":
            ...
            for h in headers:
                ...
                try:
                    ...
                    if nums:
                        res.append(f"  {h}: 非空={ne}, 数值={len(nums)}, min={min(nums):.2f}, max={max(nums):.2f}, avg={sum(nums)/len(nums):.2f}")
                    else:
            ...
            return "\n".join(res)
        if action == "filter":
            if not query: return "[csv-toolkit] filter 需要 query='col op value'"
            parts = query.split(maxsplit=2)
            if len(parts)<3: return "[csv-toolkit] query格式: 'col op value'"
            col,op,val = parts[0],parts[1],parts[2]
            if col not in headers: return f"[csv-toolkit] 列'{col}'不存在,可用: {', '.join(headers)}"
            filtered=[]
            for row in rows:
                cell=row.get(col,"")
                try:
                    if op==">": m = float(cell)>float(val)
                    elif op=="<": m = float(cell)<float(val)
                    elif op==">=": m = float(cell)>=float(val)
                    elif op=="<=": m = float(cell)<=float(val)
                    elif op=="==": m = str(cell).strip()==val.strip()
                    elif op=="!=": m = str(cell).strip()!=val.strip()
                    elif op=="contains": m = val.strip().lower() in str(cell).lower()
                    else: return f"[csv-toolkit] 不支持操作符: {op}"
                except ValueError: m=False
                if m: filtered.append(row)
            ...
            if output_path and filtered:
                ...
                lines=[delimiter.join(headers)]
                for row in filtered[:10]: lines.append(delimiter.join(str(row.get(h,"")) for h in headers))
                s+="\n"+("\n".join(lines))
            return s
