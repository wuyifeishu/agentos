...
def run(action: str, text: str = "", file_path: str = "", algorithm: str = "sha256") -> str:
    ...
    try:
        if action == "hash":
            ...
            elif algo == "sha256": h = hashlib.sha256(data).hexdigest()
            elif algo == "sha512": h = hashlib.sha512(data).hexdigest()
            else: return f"[encryption] 不支持的算法: {algorithm}, 可用: md5/sha1/sha256/sha512"
