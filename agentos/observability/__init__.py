...
class NoopTracer(Tracer):
    """No-op tracer — compatible replacement."""
    ...
    def mark_complete(self, span_id: str = ""):
        pass
...
class CostAnalytics:
    ...
    def record_call(self, prompt_tokens: int, completion_tokens: int, cost: float):
        self.prompt_tokens += prompt_tokens
        self.completion_tokens += completion_tokens
        self.total_cost += cost
        self.cost_per_call.append(cost)
...
class BudgetAlert:
    ...
    def check(self) -> Optional[str]:
        ...
        if pct >= self.threshold_pct:
            return f"Budget alert: {pct:.1f}% of ${self.budget_limit:.2f} spent"
        return None
