...
Schedule = Union[CronSchedule, IntervalSchedule, float]
...
class BackgroundExecutor:
    ...
    async def start(self, num_workers: int = 4) -> None:
        ...
        if self._scheduled:
            self._scheduler_task = asyncio.create_task(
                self._scheduler(), name="bg-scheduler"
            )
        logger.info(
            "BackgroundExecutor started: workers=%d, scheduled=%d",
            num_workers, len(self._scheduled),
        )
    ...
    async def submit_async(self, func: Callable, *args: Any,
                           retry_policy: Optional[RetryPolicy] = None,
                           timeout: Optional[float] = None,
                           task_id: Optional[str] = None,
                           **kwargs: Any) -> str:
    ...
    def scheduled(self, every: Schedule, retry_policy: Optional[RetryPolicy] = None):
        """Decorator for recurring scheduled tasks."""
        def decorator(fn):
            td = TaskDef(
                func=fn, retry_policy=retry_policy, schedule=every,
            )
    ...
    async def wait_for(self, task_id: str, timeout: Optional[float] = None) -> TaskResult:
        ...
        while True:
            ...
            if result and result.state not in (TaskState.PENDING, TaskState.RUNNING, TaskState.RETRYING):
                return result
                ...
                if td.retry_policy:
                    ...
                    if attempt < td.retry_policy.max_retries:
                        ...
                        logger.debug(
                            ...
                            td.task_id, attempt + 1, td.retry_policy.max_retries,
                            delay, exc,
                        )
        ...
        while not self._shutting_down:
            ...
            for schedule, td in self._scheduled:
                ...
                if next_run is not None and now >= next_run:
                    ...
                    await self._queue.put(TaskDef(
                        func=td.func, args=td.args, kwargs=td.kwargs,
                        task_id=f"{td.task_id}-{int(now)}",
                        retry_policy=td.retry_policy, timeout=td.timeout,
                        name=td.name,
                    ))
            await asyncio.sleep(0.1)

    @staticmethod
    def _next_run(schedule: Schedule, last: float, now: float) -> Optional[float]:
...
_default_executor: Optional[BackgroundExecutor] = None
