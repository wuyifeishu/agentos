...
class ValidationResult:
    ...
    normalized_output: Optional[Any] = None
...
class ToolOutputValidator:
    ...
    def add_rule(self, field: str, rule: ValidationRule, **kwargs) -> "ToolOutputValidator":
        ...
        if rule == ValidationRule.TYPE_CHECK:
            self._field_schemas[field] = {"type": kwargs.get("expected_type")}
        elif rule == ValidationRule.RANGE_CHECK:
            self._field_schemas[field] = {
                "min": kwargs.get("min"),
                "max": kwargs.get("max")
            }
        elif rule == ValidationRule.PATTERN_MATCH:
            self._field_schemas[field] = {"pattern": kwargs.get("pattern")}
        elif rule == ValidationRule.ENUM_CHECK:
            self._field_schemas[field] = {"allowed_values": kwargs.get("allowed_values")}
    ...
    def _parse_output(self, output: str) -> Union[Any, ValidationIssue]:
        """解析工具输出。"""
        # 尝试解析为 JSON
        try:
    ...
    def _apply_rules(self, result: ValidationResult, data: Any) -> None:
        ...
        for field, rules in self._rules.items():
            ...
            for rule in rules:
                if rule == ValidationRule.TYPE_CHECK:
                    expected_type = self._field_schemas[field]["type"]
                    if not isinstance(value, expected_type):
                        result.add_issue(ValidationIssue(
                            rule=ValidationRule.TYPE_CHECK,
                            severity=ValidationSeverity.ERROR,
                            message=f"字段类型错误: {field}",
                            field=field,
                            expected=expected_type.__name__,
                            actual=type(value).__name__,
                            suggestion=f"字段 '{field}' 应为 {expected_type.__name__} 类型"
                        ))
                
                elif rule == ValidationRule.RANGE_CHECK:
                    schema = self._field_schemas[field]
                    if "min" in schema and value < schema["min"]:
                        result.add_issue(ValidationIssue(
                            rule=ValidationRule.RANGE_CHECK,
                            severity=ValidationSeverity.WARNING,
                            message=f"字段值过小: {field}",
                            field=field,
                            expected=f">= {schema['min']}",
                            actual=value,
                            suggestion=f"字段 '{field}' 应大于等于 {schema['min']}"
                        ))
                    if "max" in schema and value > schema["max"]:
                        result.add_issue(ValidationIssue(
                            rule=ValidationRule.RANGE_CHECK,
                            severity=ValidationSeverity.WARNING,
                            message=f"字段值过大: {field}",
                            field=field,
                            expected=f"<= {schema['max']}",
                            actual=value,
                            suggestion=f"字段 '{field}' 应小于等于 {schema['max']}"
                        ))
                
                elif rule == ValidationRule.PATTERN_MATCH:
                    pattern = self._field_schemas[field]["pattern"]
                    if not re.match(pattern, str(value)):
                        result.add_issue(ValidationIssue(
                            ...
                            actual=value,
                            suggestion=f"字段 '{field}' 应符合正则表达式: {pattern}"
                        ))
                
                elif rule == ValidationRule.ENUM_CHECK:
                    allowed = self._field_schemas[field]["allowed_values"]
                    if value not in allowed:
                        result.add_issue(ValidationIssue(
                            rule=ValidationRule.ENUM_CHECK,
                            severity=ValidationSeverity.ERROR,
                            message=f"字段值不在允许范围内: {field}",
                            field=field,
                            expected=allowed,
                            actual=value,
                            suggestion=f"字段 '{field}' 应为以下值之一: {allowed}"
                        ))
...
class ToolErrorClassifier:
    ...
    def classify(tool_result: ToolResult) -> ErrorCategory:
        ...
        if tool_result.error:
            ...
            elif any(kw in error_msg for kw in ["authentication", "auth", "api key", "invalid key", "unauthorized"]):
                return ErrorCategory.AUTH
...
def validate_tool_output(tool_name: str, tool_result: ToolResult, 
    ...
    if expected_schema:
        for field, schema in expected_schema.items():
            if "type" in schema:
                validator.add_rule(field, ValidationRule.TYPE_CHECK, 
                                 expected_type=schema["type"])
            if "required" in schema and schema["required"]:
                validator.add_rule(field, ValidationRule.REQUIRED_FIELD)
            if "pattern" in schema:
                validator.add_rule(field, ValidationRule.PATTERN_MATCH,
                                 pattern=schema["pattern"])
            if "enum" in schema:
                validator.add_rule(field, ValidationRule.ENUM_CHECK,
                                 allowed_values=schema["enum"])
            if "min" in schema or "max" in schema:
                validator.add_rule(field, ValidationRule.RANGE_CHECK,
                                 min=schema.get("min"),
                                 max=schema.get("max"))
